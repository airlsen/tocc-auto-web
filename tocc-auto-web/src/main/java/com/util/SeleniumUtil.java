...
//控件操作方法---------------------------------------------------------------------------------------
	...
	public void click(WebElement element) {
		...
		try {
			...
			logger.error("The element you clicked:[" + switchtoString(element) + "] is no longer exist!");
			Assert.fail("The element you clicked:[" + switchtoString(element) + "] is no longer exist!");
		} catch (Exception e) {
			logger.error("Failed to click element [" + switchtoString(element) + "]");
			Assert.fail("Failed to click element [" + switchtoString(element) + "]",e);
		}
		logger.info("点击元素 [By." + switchtoString(element) + "]");
	}
	...
	public void clickTheClickable(WebElement element, long startTime, int timeOut) throws Exception {
		try {
			...
			if (System.currentTimeMillis() - startTime > timeOut) {
				logger.warn("["+switchtoString(element)+ "]"+" is unclickable");
				throw new Exception(e);
			} else {
				Thread.sleep(1000);
				logger.warn("["+switchtoString(element) + "]"+ " is unclickable, try again");
...
//get信息操作方法--------------------------------------------------------------------------------------
	...
	public void sendKeys(WebElement element, String key) {
		try {
			...
			e.printStackTrace();
			logger.error("输入 [" + key + "] 到 元素[By." + switchtoString(element) + "]失败");
			Assert.fail("输入 [" + key + "] 到 元素[By." + switchtoString(element) + "]失败");
		}
		logger.info("输入：[" + key + "] 到 [By." + switchtoString(element) + "]");
	}
...
//模拟action操作方法---------------------------------------------------------------------------------------------
	...
	public void pressKeysOnKeyboard(WebElement element, Keys key, String keyword) {
	   ...
	   /**
	 	 ...
	 	 * */
	 	public void WaitForElement(WebDriver driver,int timeOut, final WebElement element) {
...
//等待操作方法---------------------------------------------------------------------------------- 
		...
		public void pause(int sleepTime) {
	 	...
	 	/**
		 * 在给定的时间内去查找元素，如果没找到则超时，抛出异常,以by为参数
		 * */
...
//结果判断操作------------------------------------------------------------------------------
	...
	public boolean isDisplayed(WebElement element) {
		...
		if (element.isDisplayed()) {
			logger.info("The element: [" + switchtoString(element) + "] is displayed");
			isDisplay = true;
		} else if (element.isDisplayed() == false) {
			logger.info("The element: [" + switchtoString(element) +"] is not displayed");
...
//
	 ...
	 * 模拟键盘操作  
	 */
  public void Keyboard(By by ,Keys key){
	//显示等待
	  WebWait.WaitForElement(driver,by);
