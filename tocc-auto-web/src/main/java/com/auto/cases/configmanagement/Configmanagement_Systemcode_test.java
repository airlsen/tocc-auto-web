...
import com.auto.business.configmanagement.Systemcodebusiness;
import com.config.GetTestProperties;
import com.util.RobotScreenShot;
/**Success
 * 配置管理--数据字典管理--系统代码管理--功能测试---------------------
 * @author John
 */
public class Configmanagement_Systemcode_test extends BaseParpare{	
	//业务对象
	public static Systemcodebusiness config =null;
	//日志对象
	private static Logger log = Logger.getLogger(Configmanagement_Systemcode_test.class.getName());
	//构造函数传递weburl登录地址
    ...
    public void add(Map<String, String> data) throws Exception{
    	...
    	config=new Systemcodebusiness(webdriver,targetPath(this.getClass().getName()));	
		//断言
		try {
