package com.incito.excelReporter.utils;
import java.io.File;
...
public class CreateExcelForResult{
	 ...
	 * */
	public static void createExcelReport(List<XmlSuite> xml, List<ISuite> suites,String dir) throws Exception {
		//定义excel报告的存储目录
		String excelReportDirStr = dir+File.separator+"excelReports";
		
		System.out.println("777777777777"+excelReportDirStr);
		...
		dir = File.separator+excelReportDirStr+File.separator+"TestReport_"+fileNameFormat.format(new Date())+"-"+suites.get(0).getName()+".xls";
		System.out.println("777777777777"+dir);
	    fileForExcel = new File(dir);
	    System.out.println("888888888888"+fileForExcel);
	   		...
	   		for (ISuiteResult r : tests.values()) {//多少个模块第一层循环
    			...
    			if(failTestNum!=0){//处理失败的
				 ...
				 for (ITestNGMethod im : overview.getFailedTests().getAllMethods()) {	//模块下有多少个用例循环
 					...
 					String functionPack = caseName.substring(0, caseName.indexOf("T")).toLowerCase();
 					//设置用例名字
    			...
    			if(passTestNum!=0){//处理成功的
    				XSSFCellStyle xcs = workbook.createCellStyle();
    				System.out.println("00000000"+overview.getPassedTests().getAllMethods());
    				
    				
      				 //for (ITestNGMethod im : overview.getPassedTests().getAllMethods()) {	//模块下有多少个用例循环
      					//System.out.println("121212121212121"+im);
      					//String caseName = im.getTestClass().getRealClass().getSimpleName();
      					
    				String caseName = "SystemLoginTest";
      					String functionPack = caseName.substring(0, caseName.indexOf("T")).toLowerCase();
      					System.out.println("111111111111"+workbook);
      					System.out.println("222222222222"+function);
      					System.out.println(count+2+failTestNum+skipTestNum);

      					System.out.println("66666666666"+caseName);
      					
      				
      					System.out.println(workbook.getSheet(function));
